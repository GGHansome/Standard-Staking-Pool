...
### 3.2 锁仓与奖励加速
用户在质押资金时，必须选择一种仓位类型：活期，或某个已配置锁仓档位。
...
- **有效邀请人**：邀请人必须已完成首次质押绑定（`hasSetInviter[inviter] == true`），且非零、非本人。满足条件时永久绑定该上级。
...
- **开启**：正常配置如 `5%, 10%, 5%, 2%`。
- **灵活降级**：三级返佣比例必须构成从 L1 开始的连续前缀，即 `level2 > 0` 必须 `level1 > 0`、`level3 > 0` 必须 `level2 > 0`。合法降级配置为：仅一级 `5%, 10%, 0, 0`、仅两级 `5%, 10%, 5%, 0`、完整三级 `5%, 10%, 5%, 2%`。禁止出现「跳级空洞」（如 `level1 == 0 且 level2 > 0`，或 `level2 == 0 且 level3 > 0`），此类配置在构造函数 revert（`InvalidReferralRateConfig`）。发放侧自上而下逐级计提，遇到首个为 0 的层级即停止后续层级，与该前缀约束一致。本约束亦满足部分地区对分销层级深度的合规要求（仅一级或仅两级）。
...
- **推荐返佣层级连续性校验**：构造函数必须校验 `level1/level2/level3` 构成从 L1 开始的连续前缀，即 `level2 > 0 ⟹ level1 > 0` 且 `level3 > 0 ⟹ level2 > 0`，否则 revert（`InvalidReferralRateConfig`）。该约束消除「预算按求和预留、发放侧因 `level1 == 0` 短路整条链」导致的补贴预算被扣却永不发放的资金沉淀。`inviteeBoost` 与三级返佣相互正交：受益人为被邀请人本人、不消费返佣链，可独立于 `level*` 配置（`boost-only` 为合法的纯获客补贴形态），二者互不强制。
...
### 7.3 链上 Gas 与 DOS 防御
三级推荐的计算仅包含数次简单的乘法和判断。返佣比例构成从 L1 开始的连续前缀，发放侧自上而下逐级计提，遇到首个为 0 的层级即停止后续层级（“前缀短路”），避免无意义计算。
...
- **环形邀请结构性排除**：邀请关系是 append-only 森林——每个地址仅能绑定一次（`hasSetInviter` 去重）、绑定后 `inviterOf` 不可修改、且邀请人必须已完成绑定。用户首次绑定时 `hasSetInviter[user] == false`，此刻它不可能是任何已存在邀请链的祖先，因此新绑定无法形成环。无环性由绑定流程的结构不变量保证，合约无需运行期环检测，也不消耗 Gas 做向上追溯。注意：该结论以「`inviterOf` 永不改写」为前提；若未来引入改绑/换上级能力，必须重新引入环检测。
